...
LOCAL_LIBRARIES = $(XLIB)
  SYS_LIBRARIES = -lm
PROGRAMS = ncube
SRCS = ncube.c ncubex.c
OBJS = ncube.o ncubex.o
...
install:: ncube
	...
	$(INSTALL) $(INSTALLFLAGS) $(INSTPGMFLAGS)  ncube $(DESTDIR)$(BINDIR)/ncube

all:: ncube.$(MANNEWSUFFIX)

ncube.$(MANNEWSUFFIX): ncube.$(MANSRCSUFFIX)
	$(RM) $@
	if test -z "$(PREPROCESSMANPAGES)" ; then \
	   cd `dirname ncube` && \
	   $(LN) `basename ncube.$(MANSRCSUFFIX)` `basename $@`; \
	else \
	      	     $(RAWCPP)  $(MANDEFS) $(EXTRAMANDEFS) \
	     < ncube.$(MANSRCSUFFIX) | sed -e '/^#  *[0-9][0-9]*  *.*$$/d' 			-e '/^#line  *[0-9][0-9]*  *.*$$/d' 			-e '/^[ 	]*XCOMM$$/s/XCOMM/#/' 			-e '/^[ 	]*XCOMM[^a-zA-Z0-9_]/s/XCOMM/#/' 			-e '/^[ 	]*XHASH/s/XHASH/#/' 			-e '/\@\@$$/s/\@\@$$/\\/' >$@; \
	fi

cleandir::
	$(RM) ncube.$(MANNEWSUFFIX)

install.man:: ncube.$(MANNEWSUFFIX)
	...
	$(INSTALL) $(INSTALLFLAGS) $(INSTMANFLAGS) ncube.$(MANNEWSUFFIX) $(DESTDIR)$(MANDIR)/ncube.$(MANSUFFIX)
